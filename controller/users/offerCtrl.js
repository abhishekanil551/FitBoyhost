  ...
  try {
    ...
    const validOffers = offers.map((offer) => ({
      ...
      products: Array.isArray(offer.products)
        ? offer.products.filter((p) => p)
        : [],
      categoryId: Array.isArray(offer.categoryId)
        ? offer.categoryId.filter((c) => c)
        : [],
    }));

    const offerCategoryIds = validOffers
      .filter(
        (offer) => offer.type === "category" && offer.categoryId.length > 0
      )
    ...
    const allProducts = await Product.find({
      ...
      .select(
        "name regularPrice salesPrice poster banners categoryId isRecommended offer"
      )
      .lean();
    ...
    const enhancedProducts = allProducts
      ...
      .map((product) => {
        ...
        validOffers.forEach((offer) => {
          if (
            ...
            currentPrice = Math.floor(
              product.regularPrice * (1 - discount / 100)
            );
            isOnSale = true;
          }
          ...
          if (
            ...
            const offerCategoryIds = offer.categoryId.map((cat) =>
              cat._id.toString()
            );

            if (
              productCategoryIds.some((id) => offerCategoryIds.includes(id))
            ) {
              discount = Math.max(discount, offer.discountPercentage);
              currentPrice = Math.floor(
                product.regularPrice * (1 - discount / 100)
        ...
        const isInCart = userData
          ? userData.cart.some((cartItem) => cartItem.toString() === productId)
          : false;
        const isInWishlist = userData
          ? userData.wishlist.some(
              (wishItem) => wishItem.toString() === productId
            )
          : false;
    ...
    try {
      res.status(StatusCodes.NOT_FOUND).render("pageNotFount", {
        message: "Unable to load offers. Please try again later.",
      });
    } catch (viewError) {
      res
        .status(StatusCodes.INTERNAL_SERVER_ERROR)
        .send("Server error: Unable to load offers or render error page.");
    }
  }
};
