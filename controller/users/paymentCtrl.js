  ...
  try {
    ...
    if (!productIds || !Array.isArray(productIds) || productIds.length === 0) {
      ...
      if (!user.cart || user.cart.length === 0) {
        return res
          .status(StatusCodes.BAD_REQUEST)
          .json({ message: "No products in cart" });
      }
    ...
    if (addressParts.length !== 4) {
      return res
        .status(StatusCodes.BAD_REQUEST)
        .json({ message: "Invalid address format" });
    }
    ...
    if (couponCode) {
      ...
      if (now < coupon.startingDate || now > coupon.expiryDate) {
        return res
          .status(StatusCodes.BAD_REQUEST)
          .json({ message: "Coupon is expired or not yet valid" });
      }
      if (total < coupon.minimumAmount) {
        return res.status(StatusCodes.BAD_REQUEST).json({
          message: `Minimum purchase ₹${coupon.minimumAmount} required`,
        });
      }
    ...
    if (expectedSignature === razorpay_signature) {
      ...
      if (rewardAmount > 0) {
        await User.findByIdAndUpdate(order.userId._id, {
          ...
          $set: {
            reasonForGetPoints: `Reward points credited (${rewardAmount}) for purchase of ₹${order.total.toFixed(2)}`,
          },
        });
      }
...
`;
      ...
      pdfStream.on("end", async () => {
        ...
        try {
          ...
          res.json({
            success: true,
            message:
              "Payment successful, but failed to send invoice email. Please download your invoice from the orders page.",
          });
        }
      });
      pdfStream.on("error", (err) => {
        ...
        res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({
          success: false,
          message: "Failed to process payment due to invoice generation error",
        });
      });
    } else {
...
const orderSuccess = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res
        .status(StatusCodes.UNAUTHORIZED)
        .send("Please log in to view order success");
    }
